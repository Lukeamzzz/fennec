...
const PortafolioChart: React.FC<PortafolioChartProps> = ({
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm">
      ...
      <div>
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Resumen por Tipo de Propiedad</h3>
        
        {properties.map((property, index) => (
          <div key={index} className="flex justify-between items-center mb-3">
            <div>
              <span className="font-medium">{property.type}</span>
              <span className="text-gray-500 ml-2 text-sm">{property.units} unidades</span>
            </div>
            <div className="flex items-center">
              <span className="font-semibold mr-4">${property.value.toFixed(1)}M</span>
              <span className={`px-2 py-1 rounded text-xs ${property.percentYield >= 7 ? 'bg-green-100 text-green-800' : property.percentYield >= 6 ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>
